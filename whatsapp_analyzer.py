...
def process_ocr_result(ocr_result, participant_names):
    ...
    for _, row in ocr_result.iterrows():
        ...
        if left == 555:
            print(
                f"Skipping centered text: text: '{text}', confidence: {conf}, left: {left}"
            )
            continue
        ...
        if conf < 60:
            if should_skip_text(text, conf, left):
                continue
            print(
                f"Warning: Low confidence text: '{text}', confidence: {conf}, left: {left}"
            )

        if not current_participant:
            current_participant = determine_participant(
                left, left_threshold, participant_names
            )
...
def handle_low_confidence_word(text, conf):
    ...
    if similarity >= 0.5:  # Adjust this threshold as needed
        print(
            f"Low confidence word '{text}' (conf: {conf}). Closest optical match: '{closest_match}' (similarity: {similarity:.2f})"
        )
...
def should_skip_text(text, conf, left):
    if re.match(r"^\W+$", text):
        print(
            f"Skipping low confidence garbage text: '{text}', confidence: {conf}, left: {left}"
        )
        return True
    if conf < 30:
        print(
            f"Skipping low confidence text: '{text}', confidence: {conf}, left: {left}"
        )
        return True
    if left > 1000:
        print(
            f"Skipping low confidence text near right edge: '{text}', confidence: {conf}, left: {left}"
        )
        return True
    return False
...
def determine_participant(left, left_threshold, participant_names):
    ...
    return (
        ...
        else f"Participant {'A' if participant_index == 0 else 'B'}"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert WhatsApp screenshot(s) to text"
    )
    parser.add_argument(
        "image_path", help="Path to the WhatsApp screenshot image or file pattern"
    )
    parser.add_argument(
        "-o", "--output", help="Output file path", default="conversation.txt"
    )
    parser.add_argument(
        "--names",
    ...
    participant_names = (
        [name.strip() for name in args.names.split(",")] if args.names else []
    )
